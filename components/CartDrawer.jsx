    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 w-full md:w-[450px] bg-surface-bg border-l border-surface-border shadow-2xl z-[101] flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="flex items-center justify-between p-6 border-b border-surface-border/50 bg-surface-card">
          ...
          <button 
            onClick={closeCart}
            className="p-2 hover:bg-surface-border rounded-full transition-colors text-gray-600 hover:text-gray-900"
          >
            <X size={20} />
          </button>
        </div>
              ...
              {cart.map((item) => (
                <div key={item.id} className="bg-surface-card border border-surface-border rounded-xl p-4 flex flex-col relative">
                  <div className="flex justify-between items-start mb-2 pr-6">
                    <p className="text-gray-900 font-bold truncate max-w-[250px]">{item.fileName}</p>
                    <p className="text-gray-900 font-extrabold whitespace-nowrap">₹{item.price}</p>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-y-1 text-xs text-gray-600">
                    <p>Material: <span className="text-gray-800 font-medium">{item.config.material}</span></p>
                    <p>Color: <span className="text-gray-800 font-medium">{item.config.color}</span></p>
                    <p>Quality: <span className="text-gray-800 font-medium">{item.config.quality.split(' ')[0]}</span></p>
                    <p>Strength: <span className="text-gray-800 font-medium">{item.config.strength}%</span></p>
                  </div>
        ...
        {cart.length > 0 && (
          <div className="border-t border-surface-border/50 bg-surface-card p-6 pb-8 md:pb-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-3xl text-gray-900 font-black">₹{totalCost}</span>
            </div>
